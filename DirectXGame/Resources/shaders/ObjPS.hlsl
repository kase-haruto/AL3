...
//// フォグの計算
//float fogDistance = length(input.worldpos.xyz - cameraPos); // カメラからの距離
//float fogFactor = saturate((fogDistance - gFogParams.fogStart) / (gFogParams.fogEnd - gFogParams.fogStart));
//float4 fogColor = gFogParams.fogColor;
//shadecolor.rgb = lerp(shadecolor.rgb, fogColor.rgb, fogFactor); // フォグの色との補間
